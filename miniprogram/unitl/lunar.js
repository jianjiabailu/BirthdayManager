...
/**用法
 * obj.toSolar(2016, 6, 3); 农历转化公历
 * obj.toLunar(2016, 7, 6); 公历转化农历
 */
module.exports = {
  MIN_YEAR: 1891,
  MAX_YEAR: 2100,
  ...
  lunarByBetween: function(year, between) {
    ...
    lunar.date = [year, t, e];  //年 月 日
    lunar.char = [this.lunarYear(year), m, this.chineseDay(e)];   //天干地支年 月份 日
    lunar.zodiac = this.zodiacYear(year); //生肖年
    lunar.leapMonth = leapMonth;  //闰几月
    return lunar;
  },
  //中文年份
  chineseYear: function (year) {
    let yearArr = (year.toString()).split('')
    var yearHash = ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九'];
    let year_cn = '';
    for(let item of yearArr){
      year_cn += yearHash[item*1]
    }
    return year_cn + '年';
  },
  //中文月份
  ...
  //中文日期
  chineseDay: function(day) {
    var res, dateHash = ['', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十'];
    if (day <= 10) {
      res = '初' + dateHash[day];
    } else if (day > 10 && day < 20) {
      res = '十' + dateHash[day - 10];
    } else if (day == 20) {
      res = "二十";
    } else if (day > 20 && day < 30) {
      res = "廿" + dateHash[day - 20];
    } else if (day == 30) {
      res = "三十";
    }
    return res;
  ...
  // 根据阳历生日的月份和日期，计算星座。
  zodiac_sign: function (month, day){
